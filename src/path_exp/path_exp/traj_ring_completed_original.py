...
class RingController(Node):
    ...
    def __init__(self):
        ...
        self.move_group_name = "ur_manipulator"###### new movegroup, with the gun, still this dude, since we did not change the name

        #self.end_effector_link = "wrist_3_link" ############################################ ee link without the gun, need to change: urdf and here!!!!!!!!!!!!! done
        self.end_effector_link = "my_tool_tcp" ############################################ ee link with the gun 
...
############## get service to create geometry of obstacle -- here, our head
        self._apply_planning_scene_client = self.create_client(ApplyPlanningScene, '/apply_planning_scene')
        ###########
...
#################
    def add_obstacle_table(self):
        self.get_logger().info("Adding table obstacle to planning scene...")
        remove_req = ApplyPlanningScene.Request()
        remove_scene = PlanningScene()
        remove_scene.is_diff = True
        remove_co = CollisionObject()
        remove_co.id = "table_obstacle"
        remove_co.operation = CollisionObject.REMOVE
        remove_scene.world.collision_objects.append(remove_co)
        remove_req.scene = remove_scene
        remove_future = self._apply_planning_scene_client.call_async(remove_req)
        rclpy.spin_until_future_complete(self, remove_future)
        if not remove_future.result().success:
            self.get_logger().warn("Could not remove old table obstacle (it may not exist, which is OK).")
        add_req = ApplyPlanningScene.Request()
        ...
        table_shape.dimensions = [1.0, 1.5, 2.0]
        table_pose = Pose()
        table_pose.orientation.w = 1.0
        table_pose.position.x = 0.0
        table_pose.position.y = 3.0
        table_pose.position.z = 2.0 
        co.primitives.append(table_shape)
        co.primitive_poses.append(table_pose)
        co.operation = CollisionObject.ADD  #
        add_scene.world.collision_objects.append(co)
        add_req.scene = add_scene
        add_future = self._apply_planning_scene_client.call_async(add_req)
        rclpy.spin_until_future_complete(self, add_future)
        if add_future.result().success:
            self.get_logger().info("Successfully added table obstacle to planning scene.")
            return True
        else:
            self.get_logger().error("Failed to add table obstacle.")
            return False
    ######################################
    ...
    def plan_and_execute_rings(self, start_joint_goal):
        ...
        R_cyl = (53.0/2) * INCHES_TO_METERS
        a = R_cyl
        b = h_e
        a_sq = a**2
        b_sq = b**2
        OBJECT_CENTER_M = np.array([0.0, 0.8, a-0.05]) #############
        num_rings = 10
        points_per_ring = 60
        ...
        for i in range(1, num_rings): #############start at the #th ring
            ...
            for k in range(len(raw_points_for_ring) -1):
                ...
                tcp_standoff_dist = 0.15  ################ tcp to the surface: __cm
                offset_vector = unit_normal_vector*tcp_standoff_dist
                #offset_vector = unit_tangent_vector*flange_standoff_dist
                
                tool_orientation = get_orientation(normal_vector, tangent_vector)

                tool_position = p_current + OBJECT_CENTER_M- offset_vector #################### now tcp is __cm away from the head
                p = Pose()
            ...
            if not self.plan_and_execute_joint_goal(start_joint_goal, vel_scale=0.5, acc_scale=0.5):
...
def main(args=None):
    ...
    try:
        if ring_controller.wait_for_ready():
            ring_controller.get_logger().info("-----First, add the geometry of the head, for better visual")
            if not ring_controller.add_obstacle_table():
                ring_controller.get_logger().error("--failed to add the head")
                raise Exception("head geometry add failed")
            # This is our "Home" position
            start_joint_goal = [math.radians(0), math.radians(-75), math.radians(130), 
                                math.radians(-160), math.radians(90), math.radians(0)]
            
            ring_controller.get_logger().info("--- Step 1: Moving to Start/Home Position ---")
            success_to_start = ring_controller.plan_and_execute_joint_goal(start_joint_goal, vel_scale=0.5, acc_scale=0.5)
        ...
        ring_controller.get_logger().info(f"Mission failed due to exception: {e}")
